...
function List({ id, children }: { id: string; children: ReactNode }) {
  ...
  return (
    <ul
      ref={ref}
      className="absolute top-12 -right-8 z-10 flex flex-col overflow-clip rounded-md bg-stone-100 shadow-sm dark:bg-gray-800"
    >
      {children}
    </ul>
  );
...
function Button({
  ...
  return (
    <li className="flex w-full flex-col">
      <button
        onClick={onClick}
        className="group flex w-full cursor-pointer items-center gap-2 px-6 py-3 text-start tracking-tight text-gray-600 transition-all duration-150 hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-900"
      >
